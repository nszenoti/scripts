...
class ${CLASSNAME} extends Bloc<${EVENTNAME}, ${STATENAME}> {
  ${CLASSNAME}()
      : super(const ${STATENAME}(
          status: ${STATUSNAME}.initial,
        )){
        on<YourEvent>(_yourEventHandler);
    }

    // Add your attributes := field/methods

    Future<void> _yourEventHandler(
        YourEvent event,
        Emitter<${STATENAME}> emit,
    ) async {
        // todo: this is sample event handler !!
    }
}
EOF
)
...
# Define the content for the Event file
EVENT_CONTENT=$(cat <<EOF
part of '${BLOCFILENAME}';
...
final class YourEvent extends ${EVENTNAME}{}
...
# Create the bloc file inside the directory
echo "$BLOC_CONTENT" > "${FILENAME}/${BLOCFILENAME}"
echo "Created ${FILENAME}/${BLOCFILENAME}"

# Create the State file inside the directory
echo "$STATE_CONTENT" > "${FILENAME}/${STATEFILENAME}"
echo "Created ${FILENAME}/${STATEFILENAME}"

# Create the event file inside the directory
echo "$EVENT_CONTENT" > "${FILENAME}/${EVENTFILENAME}"
echo "Created ${FILENAME}/${EVENTFILENAME}"
