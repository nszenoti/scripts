...
echo "Running: fvm flutter clean && pub get && build_runner && gen-l10n"

# Flutter clean
echo -e "running flutter clean...\n"
if ! fvm flutter clean; then
    echo -e "${RED}Error: fvm flutter clean failed${NC}"
    exit 1
fi
...
if ! fvm flutter pub get; then
    echo -e "${RED}Error: fvm flutter pub get failed${NC}"
    exit 1
fi

# Build runner
echo -e "running build runner...\n"
if ! fvm dart run build_runner build -d; then
...
if ! fvm flutter gen-l10n; then
    echo -e "${RED}Error: fvm flutter gen-l10n failed${NC}"
    exit 1
fi
